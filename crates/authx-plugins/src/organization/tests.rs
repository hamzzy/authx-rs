...
async fn create_org_adds_owner() {
    ...
    let (org, membership) = s
        .create(owner, "ACME".into(), "acme".into(), None)
        .await
        .unwrap();
...
async fn invite_and_accept() {
    ...
    let owner = Uuid::new_v4();
    let (org, member) = s
        .create(owner, "Acme".into(), "acme2".into(), None)
        .await
        .unwrap();
    let invitee = Uuid::new_v4();
    let details = s
        .invite_member(org.id, "invitee@example.com".into(), member.role.id, owner)
        .await
        .unwrap();
...
async fn accept_invite_twice_fails() {
    ...
    let owner = Uuid::new_v4();
    let (org, member) = s
        .create(owner, "Acme".into(), "acme3".into(), None)
        .await
        .unwrap();
    let details = s
        .invite_member(org.id, "a@b.com".into(), member.role.id, owner)
        .await
        .unwrap();
...
async fn remove_member_works() {
    ...
    let owner = Uuid::new_v4();
    let (org, _) = s
        .create(owner, "Acme".into(), "acme4".into(), None)
        .await
        .unwrap();
