...
async fn upgrade_sets_real_email() {
    ...
    let user  = s.upgrade(guest.user.id, "real@example.com", "newpassword").await.unwrap();
...
async fn upgrade_non_guest_forbidden() {
    ...
    let real  = UserRepository::create(
        &store,
        CreateUser { email: "real@example.com".into(), username: None, metadata: None },
    )
    .await
    .unwrap();
    let s = svc(store);
    assert!(s.upgrade(real.id, "other@example.com", "newpassword").await.is_err());
}
