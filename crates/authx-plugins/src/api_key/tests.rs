...
async fn create_returns_raw_key() {
    ...
    let resp = svc
        .create(uid, None, "test".into(), vec![], None)
        .await
        .unwrap();
...
async fn list_returns_created_keys() {
    ...
    let uid = create_user(&s).await;
    let svc = ApiKeyService::new(s);
    svc.create(uid, None, "k1".into(), vec![], None)
        .await
        .unwrap();
    svc.create(uid, None, "k2".into(), vec![], None)
        .await
        .unwrap();
...
async fn revoke_removes_key() {
    ...
    let svc = ApiKeyService::new(s);
    let resp = svc
        .create(uid, None, "k".into(), vec![], None)
        .await
        .unwrap();
...
async fn authenticate_valid_key() {
    ...
    let svc = ApiKeyService::new(s);
    let resp = svc
        .create(uid, None, "k".into(), vec!["read".into()], None)
        .await
        .unwrap();
...
async fn authenticate_expired_key_fails() {
    ...
    let resp = svc
        .create(uid, None, "k".into(), vec![], Some(past))
        .await
        .unwrap();
