...
mod tests {
    ...
    fn real_key_store() -> Option<KeyRotationStore> {
        // Use the actual test keys from the signing module if available.
        // If the keys above are invalid PEM, skip — we test the logic flow only.
        let store = KeyRotationStore::new(3);
        store.add_key("v1", PRIV_PEM, PUB_PEM).ok()?;
        Some(store)
    }
